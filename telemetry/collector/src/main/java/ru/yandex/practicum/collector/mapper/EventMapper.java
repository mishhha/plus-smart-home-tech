...
public class EventMapper {
    ...
    public SensorEventAvro toAvro(SensorEventDto dto) {
        ...
        if (dto instanceof ClimateSensorEventDto c) {
            builder.setPayload(ClimateSensorAvro.newBuilder().setTemperatureC(c.getTemperatureC()).setHumidity(c.getHumidity()).setCo2Level(c.getCo2Level()).build());
        } else if (dto instanceof LightSensorEventDto l) {
            builder.setPayload(LightSensorAvro.newBuilder().setLinkQuality(l.getLinkQuality()).setLuminosity(l.getLuminosity()).build());
        } else if (dto instanceof MotionSensorEventDto m) {
            builder.setPayload(MotionSensorAvro.newBuilder().setLinkQuality(m.getLinkQuality()).setMotion(m.getMotion()).setVoltage(m.getVoltage()).build());
        } else if (dto instanceof SwitchSensorEventDto s) {
            builder.setPayload(SwitchSensorAvro.newBuilder().setState(s.getState()).build());
    ...
    public HubEventAvro toAvro(HubEventDto dto) {
        ...
        if (dto instanceof DeviceAddedEventDto added) {
            builder.setPayload(DeviceAddedEventAvro.newBuilder().setId(added.getId()).setType(DeviceTypeAvro.valueOf(added.getDeviceType())).build());
        } else if (dto instanceof DeviceRemovedEventDto removed) {
            builder.setPayload(DeviceRemovedEventAvro.newBuilder().setId(removed.getId()).build());
            ...
            builder.setPayload(ScenarioAddedEventAvro.newBuilder()
                ...
                .setConditions(added.getConditions().stream().map(c -> ScenarioConditionAvro.newBuilder()
                    .setSensorId(c.getSensorId()).setType(ConditionTypeAvro.valueOf(c.getType()))
                    .setOperation(ConditionOperationAvro.valueOf(c.getOperation())).setValue(c.getValue()).build()).toList())
                .setActions(added.getActions().stream().map(a -> DeviceActionAvro.newBuilder()
                    .setSensorId(a.getSensorId()).setType(ActionTypeAvro.valueOf(a.getType())).setValue(a.getValue()).build()).toList())
                .build());
